...
import {
    ...
    Eye, X, Plus, ShieldCheck, UserCheck, Edit
} from 'lucide-react';
...
export default function SuperAgentEnumeratorsPage() {
    ...
    const editMut = useMutation({
        mutationFn: ({ id, form }: { id: number; form: typeof editForm }) => 
            saEnumeratorApi.update(id, form),
        onSuccess: () => {
                        ...
                        <form onSubmit={handleCreate} className="p-5 space-y-4">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-slate-500 focus:border-slate-500 outline-none text-sm transition-shadow"
                                />
                                <p className="text-xs text-slate-400 mt-1">First <strong>{createForm.offer_point_threshold}</strong> points this enumerator earns go to you. Set 0 to pass all points directly.</p>
                            </div>
